...
#=== SGD
python main.py ${PYTHON_ARGS_BASE} --job_name nesterov --opt_name sgd_nesterov > /tmp/nesterov
python main.py ${PYTHON_ARGS_BASE} --job_name two_models --opt_name sgd_momentum --two_models --overshoot_factor 0.9 > /tmp/two_models
python main.py ${PYTHON_ARGS_BASE} --job_name efficient --opt_name sgd_overshoot --overshoot_factor 0.9 > /tmp/efficient
...
diff /tmp/efficient_clean /tmp/two_models_clean > /dev/null
echo "SGD efficient vs two models OK"

# 2) Test efficient implementation vs nesterov
cat /tmp/efficient_no_time | sed 's/ base_loss_1: [0-9]*.[0-9]* |//' | grep -v "Epoch" > /tmp/nesterov_no_base
