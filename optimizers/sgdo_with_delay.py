...
class SGDO(Optimizer):
    ...
    def _init_group(self, group, params, grads, momentum_buffer_list, state_steps):
            ...
            if p.grad is not None:
                ...
                if len(state) == 0:
                    state["step"] = (
                        ...
                        if group["fused"] else torch.tensor(0.0, dtype=_get_scalar_dtype())
                    )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            has_sparse_grad = self._init_group(
                group, params, grads, momentum_buffer_list, state_steps
            )
    ...
    def move_to_base(self):
        if self._first_step_done == False:
            return
        ...
        for group in self.param_groups:
            ...
            self._init_group(group, params, grads, momentum_buffer_list, state_steps)
            for i, param in enumerate(group["params"]):
                ...
                overshoot = clamp(step - overshoot_delay, 0, group["overshoot"])
                param.add_(momentum_buffer_list[i], alpha=group["lr"] * overshoot)
    ...
    for i, param in enumerate(params):
        ...
        if momentum != 0:
            ...
            if overshoot:
                ...
                grad.mul_(overshoot_old / momentum).add_(buf, alpha=1 + overshoot_new - (overshoot_old / momentum))
