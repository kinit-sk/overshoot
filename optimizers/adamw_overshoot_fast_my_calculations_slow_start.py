    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            current_overshoot = min(overshoot, step - 1)
            param.addcdiv_(exp_avg, denom, value=-lr * (current_overshoot + 1) / (1 - beta1**step))
            if step > 1:
                denom_old = (exp_avg_sq_old.sqrt() / (1 - beta2**(step-1))**0.5).add_(eps)

                previous_overshoot = min(overshoot, step - 2)
                param.addcdiv_(exp_avg, denom_old, value=-lr * (-previous_overshoot) / (beta1 - beta1**step))
                param.addcdiv_(grad, denom_old, value=-lr * previous_overshoot * (1 - beta1) / (beta1 - beta1**step))
...
def _multi_tensor_adamw(
    ...
    *,
    ...
    for (
        ...
        if capturable:
            raise Exception("AdamW overshoot for multitensor caprutable not implemented.")
            ...
            torch._foreach_addcdiv_(
                ...
                device_grads,
                exp_avg_sq_sqrt,
                [-lr * (overshoot/beta1) * (1 - beta1) / (1 - beta1 ** _get_value(step)) for step in device_state_steps] # type: ignore[arg-type]
            )
            ...
            torch._foreach_addcdiv_(
                ...
                device_exp_avgs,
                exp_avg_sq_sqrt,
                [-lr * (overshoot - overshoot/beta1 + 1) / (1 - beta1 ** (_get_value(step) + 1)) for step in device_state_steps] # type: ignore[arg-type]
            )
